...
exports.updateProperty = async (req, res, next) => {
    ...
    try {
        ...
        if (Object.keys(layoutFilesMap).length > 0) {
            ...
            for (const config of configurations) {
                if (!config?.unitType || !Array.isArray(config.subConfigurations)) continue;

                // normalize unit type: "2 BHK" -> "2BHK"
                const unitTypeKey = config.unitType.replace(/\s+/g, "");

                for (const subConfig of config.subConfigurations) {
                    let carpetArea = Number(subConfig.carpetArea);
                    if (!Number.isFinite(carpetArea)) continue;

                    // 1352 -> 13
                    const carpetKey = Math.floor(carpetArea / 100);
