...
exports.registerSuperAdmin = async (req, res, next) => {
  try {
    ...
    if (existingUser) {
      logInfo("Super admin registration attempt with existing email", {
        email,
      });
    ...
    logInfo("Super admin registered successfully", {
      userId: user._id,
      email: user.email,
    });
  ...
  try {
    ...
    const parsePriceToNumber = (priceStr) => {
      ...
      if (priceStrLower.includes("lakh") || priceStrLower.includes("l")) {
        ...
        priceStrLower.includes("cr") ||
        priceStrLower.includes("crore")
      ) {
  ...
  layoutImagesMap.get(key).push(file);
    ...
    if (req.files) {
      Object.keys(req.files).forEach((key) => {
        console.log(
          `req.files[${key}]:`,
          Array.isArray(req.files[key]) ? req.files[key].length : "single file",
        );
      });
    }
    ...
    const projectManagerRole = await Role.findOne({
    ...
    if (!projectManagerRole) {
      return res
        .status(400)
        .json({ success: false, message: "Role 'project_manager' not found" });
    }
    ...
    if (relationshipManager) {
      ...
      if (!rm) {
        logInfo("Property creation failed - invalid RM", {
          relationshipManager,
        });
        ...
        return res.status(400).json({
          ...
          message:
            "Invalid Relationship Manager (must have role 'project_manager')",
        });
      }
    }
    ...
    if (!agentRole) {
      return res
        .status(400)
        .json({ success: false, message: "Role 'agent' not found" });
    }

    if (
      leadDistributionAgents &&
      Array.isArray(leadDistributionAgents) &&
      leadDistributionAgents.length > 0
    ) {
      const validAgentIds = leadDistributionAgents.filter((id) =>
  ...
  subConfig.layoutPlanImages = urls;
            ...
            if (layoutImagesMapping && layoutImagesMapping[config.unitType]) {
              ...
              if (unitTypeMapping[subConfig.carpetArea]) {
                const mappedImages = Array.isArray(
    ...
    const calculateDiscountPercentage = (devPrice, offerPrice) => {
      ...
      const devPriceNum =
        typeof devPrice === "number" ? devPrice : parsePriceToNumber(devPrice);

      const offerPriceNum =
        typeof offerPrice === "number"
          ? offerPrice
          : parsePriceToNumber(offerPrice);
    ...
    if (
      ...
      Array.isArray(createdProperty.images) &&
      createdProperty.images.length > 0
    ) {
      createdProperty.images = createdProperty.images
        .map((img, idx) => {
    ...
    if (
      ...
      createdProperty.configurations = configurations.map(
        (config, configIdx) => {
            ...
            processedConfig.subConfigurations = config.subConfigurations.map(
              (subConfig, subIdx) => {
                ...
                return {
                  ...
                  availabilityStatus:
                    subConfig.availabilityStatus || "Available",

                  layoutPlanImages:
                    layoutImages.length > 0
                      ? layoutImages
                      : dbSubConfig?.layoutPlanImages || [],
      ...
      createdProperty.configurations &&
      Array.isArray(createdProperty.configurations)
    ) {
      createdProperty.configurations = createdProperty.configurations.map(
        (config) => {
          if (
            config.subConfigurations &&
            Array.isArray(config.subConfigurations)
          ) {
            config.subConfigurations = config.subConfigurations.map(
              (subConfig) => {
                if (
                  !subConfig.layoutPlanImages ||
                  !Array.isArray(subConfig.layoutPlanImages)
                ) {
                  subConfig.layoutPlanImages = [];
                }

                subConfig.layoutPlanImages = subConfig.layoutPlanImages.filter(
    ...
    if (createdProperty.connectivity instanceof Map) {
      createdProperty.connectivity = Object.fromEntries(
        createdProperty.connectivity,
      );
    } else if (
      createdProperty.connectivity &&
      typeof createdProperty.connectivity === "object"
    ) {
    } else {
      createdProperty.connectivity = {};
    }
    ...
    const [developers, relationshipManagers, agents] = await Promise.all([
      ...
      User.find({ role: projectManagerRole._id })
        ...
        p.configurations = p.configurations.map((config) => {
          if (
            ...
            const parsePrice = (priceStr) => {
              ...
              if (
                priceStrLower.includes("lakh") ||
                priceStrLower.includes("l")
              ) {
                priceNum = priceNum * 100000;
              } else if (
                priceStrLower.includes("cr") ||
...
exports.updateProperty = async (req, res, next) => {
  ...
  try {
    ...
    const layoutFilesMap = {};

    (req.files || []).forEach((file) => {
      const match = file.fieldname.match(/^layout_(.+)$/);
      if (!match) return;

      const layoutKey = match[1];

      if (!layoutFilesMap[layoutKey]) {
        layoutFilesMap[layoutKey] = [];
      }

      layoutFilesMap[layoutKey].push(file);
    });

    console.log("layoutFilesMap =>", layoutFilesMap);
    console.log("layout keys =>", Object.keys(layoutFilesMap));
    ...
    const parsePriceToNumber = (priceStr) => {
      ...
      if (lower.includes("lakh") || lower.includes("l")) priceNum *= 100000;
  ...
  updates.totalUnits = parseInt(updates.totalUnits) || 0;
    ...
    if (updates.developerPrice || updates.offerPrice) {
      const current = await Property.findById(req.params.id)
        ...
        .lean();
      const devPrice = updates.developerPrice ?? current?.developerPrice ?? 0;

      const offerPrice = updates.offerPrice ?? current?.offerPrice ?? 0;
...
updates.images = finalImages.map((url, index) => ({
  ...
  order: index + 1,
    ...
    //     const configurations = Array.isArray(property?.configurations)
    //         ? property.configurations
    //         : [];

    //     for (const config of configurations) {
    //         if (!config?.unitType) continue;

    //         const unitType = String(config.unitType).trim();

    //         // ✅ SAFE carpet area normalization
    //         let carpetArea = Number(config.carpetArea);
    //         if (!Number.isFinite(carpetArea)) continue;

    //         carpetArea =
    //             carpetArea > 100
    //                 ? Math.floor(carpetArea / 100)
    //                 : Math.floor(carpetArea);

    //         const layoutKey = `${unitType}_${carpetArea}`;

    //         if (!layoutFilesMap[layoutKey]) continue;

    //         const uploadedUrls = [];

    //         for (const file of layoutFilesMap[layoutKey]) {
    //             try {
    ...
    //         // ✅ ENSURE subConfigurations is an ARRAY
    //         if (!Array.isArray(config.subConfigurations)) {
    //             config.subConfigurations = [];
    //         }

    //         // ✅ FIND existing subConfig by layoutKey
    //         let subConfig = config.subConfigurations.find(
    //             sc => sc.layoutKey === layoutKey
    //         );
    ...
    //                 layoutPlanImages: [],
    //             };
    //             config.subConfigurations.push(subConfig);
    //         }
...
Object.keys(req.body || {}).forEach((key) => {
  ...
  if (Array.isArray(value)) {
    ...
    if (Object.keys(layoutFilesMap).length > 0) {
      ...
      const normalizeUnitType = (val) => String(val || "").replace(/\s+/g, "");

      const normalizeCarpet = (val) => String(val || "").replace(/\D/g, ""); // IMPORTANT

      for (const config of configurations) {
        if (!config?.unitType || !Array.isArray(config.subConfigurations))
          continue;

        const unitTypeKey = normalizeUnitType(config.unitType);

        for (const subConfig of config.subConfigurations) {
          const carpetKey = normalizeCarpet(subConfig.carpetArea);
          if (!carpetKey) continue;

          const layoutKey = `${unitTypeKey}_${carpetKey}`;
          ...
          if (!Array.isArray(subConfig.layoutPlanImages)) {
            subConfig.layoutPlanImages = [];
          }
  ...
  try {
    ...
    const leads = await leadModal
      ...
      .populate({
        ...
        select:
          "projectName projectId name location configurations images developerPrice offerPrice discountPercentage possessionDate minGroupMembers",
      })
  ...
  try {
    ...
    const lead = await leadModal
      ...
      .populate({
        ...
        select:
          "projectName projectId location latitude longitude possessionDate configurations amenities images relationshipManager",
    ...
    const timelineActivities = await LeadActivity.find({ leadId: lead._id })
      ...
      .populate(
        "performedBy",
        "name email phoneNumber countryCode profileImage",
      )
    ...
    const formatTimelineDescription = (activity) => {
      ...
      const performerName =
        activity.performedByName || performer.name || "Admin";
      ...
      if (activity.activityType === "phone_call") {
        ...
        description =
          ...
          `${formattedDate} - Activity by ${performerName}`;
      }
    ...
    if (lead.scheduleDate) {
      ...
      const upcomingFollowUp = timelineActivities
        ...
        .filter(
          (a) =>
            a.nextFollowUpDate && new Date(a.nextFollowUpDate) > new Date(),
        )
    ...
    logError("Error fetching lead details", error, {
  ...
  try {
    ...
    const leads = await leadModal
      ...
      .populate({
        ...
        select:
          "projectName projectId location possessionDate configurations amenities relationshipManager",
    ...
    const csvData = leads.map((lead) => {
      ...
      const configurationsFormatted = (property.configurations || [])
        .map((cfg) => {
          if (typeof cfg === "number") {
            if (cfg >= 10000000) return `₹ ${(cfg / 10000000).toFixed(2)} Cr`;
      ...
      return {
        ...
        RM_Name:
          ...
          lead.relationshipManagerId?.name ||
          "N/A",

        RM_Email:
          property.relationshipManager?.email ||
          lead.relationshipManagerId?.email ||
          "N/A",

        RM_Phone:
          property.relationshipManager?.phone ||
          lead.relationshipManagerId?.phone ||
          "N/A",
    ...
    logInfo("All leads exported to CSV", {
      userId,
    ...
    logError("Error exporting all leads to CSV", error, {
      userId: req.user?.userId,
    });
...
const createNotification = async (
  ...
  try {
    const lead = await leadModal
      ...
      .populate(
        "propertyId",
        "relationshipManager leadDistributionAgents projectName projectId",
      )
    ...
    if (
      property.leadDistributionAgents &&
      Array.isArray(property.leadDistributionAgents)
    ) {
...
exports.updateLeadStatus = async (req, res) => {
  try {
    ...
    const updatedLead = await leadModal
      .findByIdAndUpdate(
        leadId,
    ...
    const leadForNotification = await leadModal
      .findById(leadId)
      .populate("userId", "name")
      .lean();
  ...
  try {
    ...
    const formattedTimeline = activities.map((activity) => {
      ...
      return {
        ...
        formattedDate: new Date(activity.activityDate).toLocaleDateString(
          "en-IN",
          {
            day: "numeric",
    ...
    logError("Error fetching lead timeline", error, {
      leadId: req.params.leadId,
    });
...
exports.getAdminDashboard = async (req, res, next) => {
  try {
    ...
    const [totalDevelopers, liveProjects, totalLeads, totalBookingsThisMonth] =
      await Promise.all([
    ...
    const recentLeads = await leadModal
      .find({ isStatus: true })
      ...
      .populate({
        ...
        select:
          "projectName projectId developerPrice offerPrice discountPercentage images",
      })
    ...
    const salesTeamRoleIds = [agentRole?._id, projectManagerRole?._id].filter(
      Boolean,
    );
    ...
    const salesTeamPerformance = await Promise.all(
      salesTeamMembers.map(async (member) => {
    ...
    allProperties.forEach((property) => {
      if (property.configurations && property.configurations.length > 0) {
        property.configurations.forEach((config) => {
          ...
          if (
            config.availabilityStatus === "Sold" ||
            config.availabilityStatus === "Reserved"
          ) {
            confirmedUnits++;
          }
        });
...
exports.getCRMDashboard = async (req, res, next) => {
  try {
    ...
    const allLeads = await leadModal
      .find({
        ...leadFilter,
    ...
    if (allLeadIds.length > 0) {
      const contactActivities = await LeadActivity.find({
        ...
        .select("leadId activityDate")
        .lean();

      const contactedLeadIdSet = new Set(
        contactActivities
          .map((activity) => activity.leadId?.toString())
          .filter(Boolean),
      );

      contactedLeadIds = Array.from(contactedLeadIdSet).map(
      ...
      if (contactedLeadIds.length > 0) {
        const firstContacts = await LeadActivity.aggregate([
          {
            $match: {
              ...
              activityType: {
                $in: ["phone_call", "whatsapp", "email", "visit"],
              },
            },
          },
        ...
        for (const contact of firstContacts) {
          const lead = allLeads.find(
            (l) => l._id.toString() === contact._id.toString(),
          );

          if (lead && contact.firstContactDate) {
            const responseTimeMs =
              new Date(contact.firstContactDate) - new Date(lead.createdAt);
        ...
        avgResponseTimeHours =
          count > 0 ? Math.round(totalResponseTime / count) : 0;
      }
    }

    const contactedLeads = contactedLeadIds.length;

    const leadsContactedPercentage =
      totalLeads > 0 ? Math.round((contactedLeads / totalLeads) * 100) : 0;
    ...
    if (sortBy === "name_asc" || sortBy === "name_desc") {
      ...
      const populatedLeads = await Promise.all(
        leadsAggregation.map(async (item) => {
    ...
    const leadIds = leads
      .map((lead) => lead._id || lead._id?.toString())
      .filter(Boolean);
    ...
    const formatTimelineEntry = (
      ...
      const date = new Date(
        entry.timestamp || entry.activityDate || entry.createdAt,
      );
    ...
    const formattedLeads = await Promise.all(
      leads.map(async (lead) => {
        ...
        const activities = (activitiesByLeadId[leadIdStr] || []).sort(
        ...
        activities.forEach((activity) => {
          ...
          if (
            activity.activityType === "lead_received" ||
            activity.activityType === "join_group"
          ) {
            return;
          }
          ...
          if (activity.activityType === "visit") {
            ...
            if (existingVisit) {
              const existingDate = existingVisit.visitDate
                ? new Date(existingVisit.visitDate).toDateString()
                : null;

              const existingTime = existingVisit.visitTime || null;

              const newDate = activity.visitDate
                ? new Date(activity.visitDate).toDateString()
                : null;
              ...
              if (existingDate !== newDate || existingTime !== newTime) {
                const visitDateTime =
                  activity.visitDate && activity.visitTime
                    ? `${new Date(activity.visitDate).toLocaleDateString(
                        "en-IN",
                        {
                          day: "numeric",
    ...
    logError("Error fetching CRM dashboard", error, {
      userId: req.user?.userId,
    });
  ...
  try {
    ...
    if (followUpDate) {
      ...
      if (isNaN(parsedFollowUpDate.getTime())) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid follow-up date format" });
      }
    ...
    const activityDescription = parsedFollowUpDate
      ? `Follow up scheduled for ${parsedFollowUpDate.toLocaleDateString(
          "en-IN",
          {
            day: "numeric",
    ...
    if (parsedFollowUpDate) {
      await createNotification(
        ...
        `This is a reminder for the upcoming follow-up regarding:\nProject: ${property.projectName || "N/A"}\nProject ID: ${property.projectId || "N/A"}\nNext Follow-up Date: ${parsedFollowUpDate.toLocaleDateString(
          "en-IN",
          {
            day: "numeric",
...
exports.updateBlog = async (req, res, next) => {
  try {
    ...
    if (req.files?.bannerImage && req.files.bannerImage[0]) {
      try {
        updates.bannerImage = await uploadToS3(
          req.files.bannerImage[0],
          "blogs/banner",
        );
      } catch (uploadError) {
