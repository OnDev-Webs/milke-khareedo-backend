...
const getJoinedGroupPropertyIds = async (userId) => {
  ...
  try {
    const leads = await leadModal
      ...
      .lean();
    return new Set(
      leads.map((lead) => lead.propertyId?.toString()).filter(Boolean)
    );
  } catch (error) {
...
const getBookedVisitPropertyIds = async (userId) => {
  ...
  try {
    ...
    const userLeads = await leadModal
      .find({
        userId: userId,
        isStatus: true,
        propertyId: { $exists: true, $ne: null },
...
const formatPropertyImages = (images) => {
  ...
  const otherImages = images.filter(
    (img) => !img.isCover || img.isCover === false
  );
...
const formatPropertyData = async (property, userId = null) => {
  ...
  if (property.developer?._id || property.developer?.developerName) {
    developerInfo = {
      developerName: property.developer.developerName || "N/A",
    };
  } else {
    const developerId = property.developer?.toString() || property.developer;
    if (developerId) {
      developerInfo = await Developer.findById(developerId)
    ...
    if (property.configurations && Array.isArray(property.configurations)) {
      property.configurations.forEach((config) => {
        if (
          ...
          count += config.subConfigurations.filter(
            (sc) =>
              sc.availabilityStatus === "Available" ||
              sc.availabilityStatus === "Ready"
          ).length;
        } else if (
          config.availabilityStatus === "Available" ||
          config.availabilityStatus === "Ready"
        ) {
          count += 1;
        }
      });
  ...
  if (property.discountPercentage) {
    discountPercentageValue =
      parseFloat(property.discountPercentage.replace("%", "")) || 0;
    ...
    devPriceNum > 0 &&
    offerPriceNum > 0 &&
    devPriceNum > offerPriceNum
  ) {
    discountAmount = devPriceNum - offerPriceNum;
    discountPercentageValue = parseFloat(
      ((discountAmount / devPriceNum) * 100).toFixed(2)
    );
  }
...
exports.getUserDashboard = async (req, res) => {
  try {
    ...
    logInfo("User dashboard data fetched", {
      userId,
      totalViewed,
      totalFavorited,
      totalVisited,
    ...
    logError("Error fetching user dashboard", error, {
...
exports.addViewedProperty = async (req, res) => {
  try {
    ...
    logError("Error adding property view in dashboard", error, {
      userId: req.user?.userId,
...
exports.toggleFavoriteProperty = async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      logInfo("Property removed from favorites in dashboard", {
        userId,
        propertyId,
      });
    ...
    logError("Error toggling favorite property in dashboard", error, {
      userId: req.user?.userId,
      propertyId: req.body?.propertyId,
    });
    res.status(500).json({ success: false, message: error.message });
...
exports.registerVisit = async (req, res) => {
  try {
    ...
    if (visitDate && isNaN(parsedVisitDate.getTime())) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid visitDate format" });
    }
    ...
    logError("Error registering visit in dashboard", error, {
      userId: req.user?.userId,
      propertyId: req.body?.propertyId,
    });
    res.status(500).json({ success: false, message: error.message });
...
exports.getSearchHistory = async (req, res) => {
  try {
    ...
    logInfo("Search history fetched", {
      userId,
...
exports.saveContactPreferences = async (req, res) => {
  try {
    ...
    const { preferredLocations, budgetMin, budgetMax, floorMin, floorMax } =
      req.body;
    ...
    if (
      ...
      (isNaN(budgetMinNum) || budgetMinNum < 0)
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (
      ...
      (isNaN(budgetMaxNum) || budgetMaxNum < 0)
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (
      floorMinNum !== undefined &&
      floorMaxNum !== undefined &&
      floorMinNum > floorMaxNum
    ) {
      return res.status(400).json({
        success: false,
    ...
    logError("Error saving contact preferences", error, {
...
exports.getContactPreferences = async (req, res) => {
  try {
    ...
    logError("Error fetching contact preferences", error, {
      userId: req.user?.userId || req.user?._id,
    });
    return res.status(500).json({
      success: false,
  ...
  try {
      ...
      .populate({
        ...
        populate: [
          { path: "developer", select: "developerName" },
          {
            ...
            select: "name email phone phoneNumber countryCode",
          },
        ],
        select:
          "projectName location latitude longitude configurations images developerPrice offerPrice discountPercentage minGroupMembers projectId possessionStatus developer relationshipManager possessionDate",
    ...
    const formattedProperties = await Promise.all(
      activities.map(async (activity) => {
        ...
        const formattedProperty = await formatPropertyData(
          activity.propertyId,
          userId
        );
        return {
    ...
    logError("Error fetching viewed properties", error, {
      userId: req.user?.userId,
    });
    res.json({ success: false, message: error.message });
  }
...
exports.getFavoritedProperties = async (req, res) => {
  try {
    ...
    logError("Error fetching favorited properties", error, {
      userId: req.user?.userId,
    });
    res.json({ success: false, message: error.message });
  }
...
exports.getVisitedProperties = async (req, res) => {
  try {
    ...
    leadsWithVisits.forEach((lead) => {
      ...
      if (visitActivity && visitActivity.visitDate) {
        ...
        const activityDate = new Date(
          visitActivity?.activityDate || lead.createdAt
        );
    ...
    const upcomingWithProperties = await Promise.all(
      upcomingPaginated.map(async (lead) => {
        ...
        const formattedProperty = property
          ? await formatPropertyData(property, userId)
          : null;
    ...
    logError("Error fetching visited properties", error, {
      userId: req.user.userId,
    });
    res.json({ success: false, message: error.message });
  }
